        ...
        if source.extends
          ...
          source.provides.each do |p|
            if provides_map[p] && provides_map[p] != source && provides_map[p].filename != source.filename
              Jsus::Middleware.errors << "Redeclared #{p.to_s} in #{source.filename} (previously declared in #{provides_map[p].filename})".tap {|a| Jsus.verbose? && puts(a) }
            end
    ...
    def lookup_direct_dependencies!(source)
      ...
      source.dependencies.map do |dependency|
        ...
        if (!result || (result.is_a?(Array) && result.empty?))
          Jsus::Middleware.errors << "#{source.filename} is missing #{dependency.is_a?(SourceFile) ? dependency.filename : dependency.to_s}".tap {|a| Jsus.verbose? && puts(a) }
        end
        result
